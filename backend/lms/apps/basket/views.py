...
from lms.apps.basket.signals import basket_addition, voucher_addition, voucher_removal
from lms.core import ajax
from lms.core.loading import get_class, get_classes, get_model
from lms.core.utils import is_ajax, redirect_to_referrer, safe_referrer
...
class BasketView(ModelFormSetView):
    ...
    template_name = "lms/basket/basket.html"
    ...
    def get_queryset(self):
        """
        Return list of :py:class:`Line <lms.apps.basket.abstract_models.AbstractLine>`
            ...
            if hasattr(form, "cleaned_data") and form.cleaned_data.get(
                ...
                if self.request.user.is_authenticated:
                    ...
                    msg = render_to_string(
                        "lms/basket/messages/line_saved.html", {"line": line}
                    )
    ...
    def json_response(self, ctx, flash_messages):
        basket_html = render_to_string(
            "lms/basket/partials/basket_content.html",
            context=ctx,
            request=self.request,
        )
...
class BasketAddView(FormView):
    ...
    a templatetag from :py:mod:`lms.templatetags.basket_tags`.
    """
    ...
    def get_success_message(self, form):
        return render_to_string(
            "lms/basket/messages/addition.html",
    ...
    def formset_valid(self, formset):
        ...
        for form in formset:
            if form.cleaned_data.get("move_to_basket", False):
                is_move = True
                msg = render_to_string(
                    "lms/basket/messages/line_restored.html", {"line": form.instance}
                )
