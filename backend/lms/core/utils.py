...
def default_slugifier(value, allow_unicode=False):
    """
    Lms's default slugifier function. When unicode is allowed
...
def slugify(value):
    ...
    function to use, defaults to 'lms.core.utils.default_slugifier'.
...
def format_datetime(dt, dt_format=None):
    ...
    if is_naive(dt):
        ...
        logging.warning("lms.core.utils.format_datetime received native datetime")
    else:
...
def is_ajax(request):
    ...
    For Lms projects that's usually not a concern though.
    """
