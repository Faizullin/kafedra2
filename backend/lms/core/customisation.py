...
def fork_app(label, local_folder_path, local_app_subpackage=None, logger=None):
    """
    Create a custom version of one of Lms' apps.

    The first argument is the app label of the Lms app to fork.
    ...
    try:
        ...
        if not isinstance(app_config, LmsConfig):
            raise ValueError("There is no Lms app with the label '{}'".format(label))
    ...
    if local_app_subpackage is None:
        local_app_subpackage = app_config.name.replace("lms.apps.", "")
    ...
    # Only create models.py and migrations if they exist in the Lms app
