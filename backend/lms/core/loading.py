...
# from one Lms module to another, we look into the dictionary below
# for the moved items during loading.
MOVED_MODELS = {}


def get_class(module_label, classname, module_prefix="lms.apps"):
    """
...
def get_class_loader():
    return import_string(settings.LMS_DYNAMIC_CLASS_LOADER)


def get_classes(module_label, classnames, module_prefix="lms.apps"):
...
def default_class_loader(module_label, classnames, module_prefix):
    ...
    Examples:
        ...
        lms.apps.dashboard.catalogue.forms.ProductForm
        ...
        [lms.apps.dashboard.catalogue.forms.ProductForm,
         lms.apps.dashboard.catalogue.forms.StockRecordForm]
    ...
    # import from Lms package (should succeed in most cases)
    # e.g. 'lms.apps.dashboard.catalogue.forms'
    lms_module_label = "%s.%s" % (module_prefix, module_label)
    lms_module = _import_module(lms_module_label, classnames)

    # returns e.g. 'lms.apps.dashboard.catalogue',
    ...
    if lms_module is local_module is None:
    ...
    return _pluck_classes([local_module, lms_module], classnames)
...
def _find_registered_app_name(module_label):
    """
    Given a module label, finds the name of the matching Lms app from the
    Django app registry.
    """
    from .application import LmsConfig
    ...
    if not isinstance(app_config, LmsConfig):
        raise AppNotFoundError(
            "Couldn't find an Lms app to import %s from" % module_label
        )
...
def get_profile_class():
    ...
    # makes sense for Lms to continue to use it. Projects built on Django
...
def feature_hidden(feature_name):
    """
    Test if a certain Lms feature is disabled.
    """
...
def get_model(app_label: str, model_name: Type[Model] | str):
    ...
    """
    lms_moved_model = MOVED_MODELS.get(app_label, None)
    if lms_moved_model:
        if model_name.lower() in lms_moved_model[1]:
            original_app_label = app_label
            app_label = lms_moved_model[0]
            warnings.warn(
...
def is_model_registered(app_label, model_name):
    ...
    register Lms models if they aren't overridden by a forked app.
    """
    try:
