...
def currency(value, currency_format=None):
    ...
    LMS_CURRENCY_FORMAT = getattr(settings, "LMS_CURRENCY_FORMAT", None)
    kwargs = {
        "currency": currency_format,
        "locale": to_locale(get_language() or settings.LANGUAGE_CODE),
    }
    if isinstance(LMS_CURRENCY_FORMAT, dict):
        kwargs.update(LMS_CURRENCY_FORMAT.get(currency_format, {}))
    else:
        kwargs["format"] = LMS_CURRENCY_FORMAT
