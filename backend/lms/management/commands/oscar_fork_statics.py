...
class Command(BaseCommand):
    """
    Copy Lms's statics into local project so they can be used as a base for
    styling a new site.
    """

    help = "Copy Lms's static files"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Copying Lms's static files to %s" % destination)
        shutil.copytree(source, destination)

        # Check if this new folder is in STATICFILES_DIRS
        if destination not in settings.STATICFILES_DIRS:
            self.stdout.write(
                "You need to add %s to STATICFILES_DIRS in order for your local overrides to be picked "
                "up" % destination
            )
