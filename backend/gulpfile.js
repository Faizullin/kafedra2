...
const path = {
  src: {
    // html: 'lms/static_src/html/**/*.html',
    css: 'lms/static_src/assets/scss/*.scss',
    layoutjs: 'lms/static_src/assets/js/*.js',
    pagesjs: 'lms/static_src/assets/js/pages/*.js',
    images: 'lms/static_src/assets/images/**/*.{jpg,png}'
  },
  destination: {
    // html: 'static',
    css: 'static/assets/css',
    layoutjs: 'static/assets/js',
    pagesjs: 'static/assets/js/pages',
...
gulp.task('build-node-modules', function () {
  ...
  npmlodash(required_libs).forEach(function (assets, type) {
    if (type == 'css') {
      gulp.src(assets).pipe(gulp.dest('static/assets/css/plugins'));
    } else {
      gulp.src(assets).pipe(gulp.dest('static/assets/js/plugins'));
    }
  });

  var cpyassets = gulp.src(['lms/static_src/assets/**/*.*', '!lms/static_src/assets/scss/**/*.*']).pipe(gulp.dest('static/assets'));
...
gulp.task('watch', function () {
  gulp.watch('lms/static_src/assets/scss/**/*.scss', gulp.series('sass')).on('change', browsersync.reload);
  gulp.watch('lms/static_src/assets/js/**/*.js', gulp.series('build-js')).on('change', browsersync.reload);
  // gulp.watch('lms/static_src/html/**/*.html', gulp.series('build-html')).on('change', browsersync.reload);
});
//  [ watch ] start
...
//   return gulp
//     .src([path.src.html,'!lms/static_src/html/elements/*.html'])
//     .pipe(
//       fileinclude({
//         context: layout,
//         prefix: '@@',
...
gulp.task('watch-minify', function () {
  gulp.watch('lms/static_src/assets/scss/**/*.scss', gulp.series('min-css'));
  gulp.watch('lms/static_src/assets/js/**/*.js', gulp.series('min-js'));
  // gulp.watch('lms/static_src/html/**/*.html', gulp.series('min-html'));
});
//  [ watch minify ] start
