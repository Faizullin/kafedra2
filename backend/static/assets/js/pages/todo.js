...
document.addEventListener("DOMContentLoaded", function () {
  bindclick();
  document
    .querySelector("button#create-task")
    .addEventListener("click", function () {
      ...
      if (task.length == 0) {
        ...
        document
          ...
          .insertAdjacentHTML("beforeend", newTask);
  ...
  function bindclick() {
    ...
    for (var i = 0; i < pclinkclick.length; i++) {
      document
        .querySelector("#task-list")
        .insertAdjacentHTML("beforeend", pclinkclick[i].outerHTML);
    }
    var pclinkclick = document.querySelectorAll("#task-list li");
    for (var i = 0; i < pclinkclick.length; i++) {
  ...
  document.querySelector("#add-btn").addEventListener("click", function () {
    ...
    if (task == "") {
      ...
      document
        ...
        .insertAdjacentHTML("beforeend", add_todo);
      checkstate();
...
function checkstate() {
  ...
  var changestate = document.querySelectorAll(
  ...
  .addEventListener("click", function () {
    ...
    if (task.length == 0) {
      ...
      var add_todo =
        ...
        k +
        '">' +
        '<label class="form-check-label" for="customCheckmod' +
        k +
        '">' +
        task +
        "</label>" +
